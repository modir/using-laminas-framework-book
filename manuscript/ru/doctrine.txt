...
I> Doctrine не является частью Laminas Framework, это сторонняя библиотека. Мы рассматриваем ее
I> в этой книге, потому что она предоставляет легкий способ добавления поддержки базы данных
I> в веб-приложение на базе Laminas.
...
|--------------------------------|---------------------------------------------------------------|
| `DoctrineORMModule`            | Простая интеграция между Doctrine ORM и Laminas.                  |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\Paginator`               | Поддержка пагинации.                                          |
|--------------------------------|---------------------------------------------------------------|
...
Чтобы скачать приложение *Blog* перейдите на [эту страницу](https://github.com/olegkrivtsov/using-laminas-book-samples)
...
## Интеграция ORM Doctrine и Laminas Framework

Для легкой интеграции с Laminas Framework, Doctrine предоставляет два следующих компонента
(которые, на самом деле, являются модулями Laminas):

  * [DoctrineModule](https://github.com/doctrine/DoctrineORMModule) - это модуль Laminas, который
  ...
  * [DoctrineORMModule](https://github.com/doctrine/DoctrineORMModule) предназначен для интеграции
    объектно-реляционного преобразователя (ORM) Doctrine и Laminas Framework.

Оба этих компонента распространяются в качестве пакета, устанавливаемого Composer'ом и зарегистрированного
в каталоге [Packagist.org](https://packagist.org/). Этот способ похож на тот, что использует Laminas Framework
...
Этот код сообщает Laminas, что при старте приложения нужно загружать модули DoctrineModule* и *DoctrineORMModule*.
...
Как вы уже знаете из главы [Как работает веб-сайт](#operation), конфигурация веб-приложения на базе Laminas, как правило,
...
Когда сайт на базе Laminas загружает свои настройки, он объединяет все конфигурации в один большой массив,
...
`EntityManager` зарегистрирован в качестве сервиса в менеджере сервисов Laminas Framework.
...
Как видите из этого фрагмента, класс `PostForm` определяет laminas-форму с полями заголовка
...
можем воспользваться функциональностью для пагинации, которую предоставляет нам Laminas Framework.

I> Хотя мы и планируем использовать компонент для пагинации из Laminas, мы все равно будем внутренне использовать пагинатор Doctrine ORM, чтобы подавать ему данные.
I> Пагинатор Laminas будет всего лишь оберткой для пагинатора Doctrine ORM.

### Laminas Paginator

В Laminas Framework, пагинация реализована внутри компонента @`Laminas\Paginator`. Если вы еще не установили этот компонент,
...
Компонент *Laminas\Paginator* предоставляет класс @`Paginator`. Его наиболее полезные методы представлены в таблице 12.6:

{title="Таблица 12.6. Методы класса Paginator из Laminas"}
|------------------------------------|----------------------------------------------------|
| *Метод*                            | *Описание*                                         |
|------------------------------------|----------------------------------------------------|
...
данные из пагинатора ORM и передать их в пагинатор Laminas.
...
Мы затем передаем этот объект `Query` пагинатору Laminas в строке 31.
...
Частичный шаблон представления будет использован помощником представления @`PaginationControl` Laminas, чтобы визуализировать виджет пагинатора. Как вы можете увидеть,
...
Помощник представления @`PaginationControl` принимает четыре аргумента:
  ...
  * аргумент "Sliding" - это стиль пагинатора. Для дополнительной информации о доступных стилях пагинации обратитесь к руководству Laminas Framework.
...
T> рассмотрите возможность использования jQuery-плагина [DataTables](https://datatables.net/) или [BootGrid](http://www.jquery-bootgrid.com/).

## Выводы

Хотя Doctrine не является частью Laminas Framework, мы рассматриваем примеры ее использования в
