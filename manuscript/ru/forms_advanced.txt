...
в Laminas Framework. В этой главе мы рассмотрим продвинутые темы, касающиеся
...
| *Компонент*                    | *Описание*                                                    |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\Captcha`                 | Реализует различные алгоритмы CAPTCHA.                        |
|--------------------------------|---------------------------------------------------------------|
...
Мы рассмотрим использование двух предоставляемых Laminas Framework
элементов безопасности форм: @`Captcha`[Laminas\Form\Element\Captcha] и `Csrf`[Laminas\Form\Element\Csrf] (оба класса принадлежат
пространству имен @`Laminas\Form\Element`[Laminas\Form]). Добавление этих элементов на модель
...
В Laminas Framework доступно несколько типов CAPTCHA (все принадлежат
компоненту @`Laminas\Captcha`):
...
Компонент @`Laminas\Captcha` предоставляет единообразный интерфейс для всех типов
CAPTCHA (интерфейс @`AdapterInterface`[Laminas\Captcha\AdapterInterface]). Базовый класс @`AbstractAdapter`[Laminas\Captcha\AbstractAdapter] реализует
...
[^адаптер]: *Адаптер* - это шаблон проектирования, который преобразовывает интерфейс класса в
			...
			как они все реализуют интерфейс @`AbstractAdapter`[Laminas\Captcha\AbstractAdapter], их все можно использовать
...
который использует некое секретное слово из символов: класс @`AbstractWord`[Laminas\Captcha\AbstractWord]. Этот
...
Laminas предоставляет специальный класс элемента формы и класс помощника вида для использования полей CAPTCHA на формах.

Чтобы добавить поле CAPTCHA к модели формы, используйте класс @`Captcha`[Laminas\Form\Element\Captcha], который принадлежит компоненту @`Laminas\Form`
и "живет" в пространстве имен @`Laminas\Form\Element`.

Класс элемента @`Captcha`[Laminas\Form\Element\Captcha] может быть использован с любым алгоритмом CAPTCHA (перечислены
в предыдущем разделе) из компонента @`Laminas\Captcha`. Для этой цели, класс элемента имеет
метод `setCaptcha()`, который принимает либо экземпляр класса, реализующего интерфейс
@`Laminas\Captcha\AdapterInterface`, либо массив, содержащий настройку CAPTCHA [^массив].
...
[^массив]: Во втором случае (описание в виде массива) алгоритм CAPTCHA будет
           автоматически инстанцирован и инициализирован классом фабрики @`Laminas\Captcha\Factory`.

Элемент @`Captcha`[Laminas\Form\Element\Captcha] добавляется к модели формы обычным способом, через метод `add()`,
предоставляемый базовым классом @`Laminas\Form\Form`. Как обычно, вы можете передать методу
либо экземпляр класса @`Laminas\Form\Element\Captcha`, либо массив опций конфигурации для
...
В этом фрагменте мы вызываем метод `add()`, предоставляемый базовым классом @`Form`[Laminas\Form\Form],
 ...
 * Ключ массива `type` (строка 4), как и обычно, может быть либо полностью определенным именем класса
   элемента (@`Laminas\Form\Element\Captcha`), либо его псевдонимом ("captcha").
 ...
 * Ключ `class` (строка 9) может содержать либо полное имя класса CAPTCHA (например, @`Laminas\Captcha\Image`),
...
(принадлежащий пространству имен @`Laminas\Form\View\Helper`[Laminas\Form]). Однако, как вы могли узнать из предыдущей
...
предоставляемых Laminas: @`Image`[Laminas\Captcha\Image] и @`Figlet`[Laminas\Captcha\Figlet]. Мы покажем, как добавить поле CAPTCHA к форме
...
Чтобы добавить CAPTCHA @`Image`[Laminas\Captcha\Image] к модели формы, вызовите метод формы
...
параметры для настройки алгоритма CAPTCHA @`Image`[Laminas\Captcha\Image], присоединенного к элементу формы:


 * параметр `class` (строка 21) должен быть либо полностью определенным именем класса
   адаптера CAPTCHA (@`\Laminas\Captcha\Image`), либо его псевдоним (`Image`).
...
@`Laminas\Form\Element\Captcha` сравнит отправленные буквы с теми, что хранятся в PHP-сессии.
...
параметры для настройки алгоритма CAPTCHA @`Figlet`[Laminas\Captcha\Figlet], присоединенного к элементу формы:

 * параметр `class` (строка 11) должен быть либо полностью определенным именем класса
   адаптера CAPTCHA (@`\Laminas\Captcha\Figlet`), либо его псевдоним (`Figlet`).
...
В Laminas Framework, чтобы добавить CSRF-защиту к модели формы,
используется класс элемента формы @`Laminas\Form\Element\Csrf`.

I> У элемента @`Csrf`[Laminas\Form\Element\Csrf] нет визуального представления (вы не увидите его на экране).
...
В этом фрагменте мы используем метод `add()` класса @`Form`[Laminas\Form\Form] (строка 2), которому передается
...
имя класса (@`Laminas\Form\Element\Csrf`), либо псевдоним ("csrf").
...
В строке 6, внутри массива `csrf_options`, мы указываем опции для класса @`Laminas\Form\Element\Csrf`.
...
Мы можем элегантно справиться с данной проблемой с использованием группы валидации. Класс @`Form`[Laminas\Form\Form] предоставляет метод `setValidationGroup()`,
...
В этом разделе мы приведем инструкции по реализации *многошаговой* формы с помощью Laminas. Многошаговая форма - это
...
Поддержка сессий реализована в компоненте @`Laminas\Session`, так что вам нужно установить его, если вы еще этого не сделали.
...
Laminas Framework предоставляет два класса, целью которых является повышение безопасности
форм: @`Captcha`[Laminas\Form\Element\Captcha] и `Csrf`[Laminas\Form\Element\Csrf]. CAPTCHA - это тип теста вида "вызов-ответ", используемый для определения
того, является ли пользователь человеком. Элементы CAPTCHA используются на форме для предотвращения
отправки формы вредоносным автоматизированным процессом (роботом). Второй элемент, @`Csrf`[Laminas\Form\Element\Csrf], используется
