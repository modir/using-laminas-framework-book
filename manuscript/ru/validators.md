...
В Laminas *валидатор* - это PHP-класс, реализующий интерфейс `ValidatorInterface` (он принадлежит
пространству имен `Laminas\Validator`). Определение интерфейса представлено ниже:
...
Стандартные валидаторы Laminas предоставлены компонентом `Laminas\Validator` [^std-validators].
...
[^std-validators]: В этом разделе мы считаем стандартными только те валидаторы, которые принадлежат пространству имен `Laminas\Validator`.
...
валидатор может выбросить исключение `Laminas\Validator\Exception\RuntimeException` или выдать PHP-предупреждение.
...
В Laminas Framework существует несколько способов создания валидатора:
...
это полностью определенное имя класса валидатора (например, `\Laminas\Validator\EmailAddress`), либо его
...
класса `Laminas\Validator\EmailAddress` псевдоним `EmailAddress`.
...
за разбор строки URI. По умолчанию в качестве обработчика URI используется класс `Laminas\Uri\Uri`
...
Так как Laminas не предоставляет стандартного валидатора для осуществления подобной операции фильтрации номера,
...
T> `PhoneValidator` приведен только для демонстрации того, как писать собственные валидаторы в Laminas.
T> Реализация валидатора, который бы правильно обрабатывал все возможные номера телефонов в мире
T> находится за пределами обсуждения данной книги. Если вы захотите использовать данный валидатор в своем приложении,
T> вам определенно придется улучшить его. Например, взгляните на PHP библиотеку `libphonenumber`
T> от Google.
...
В Laminas Framework существует несколько групп стандартных валидаторов:
