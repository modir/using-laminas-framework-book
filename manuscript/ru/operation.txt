...
В этой главе мы рассмотрим работу типичного веб-приложения на базе Laminas Framework.
...
ознакомитесь с такими важными компонентами Laminas как @`Laminas\EventManager`, @`Laminas\ModuleManager` и @`Laminas\ServiceManager`.
...
| *Компонент*                    | *Описание*                                                   |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\Mvc`                     | Поддержка шаблона "модель-представление-контроллер". Отделяет  |
|                                | бизнес-логику от представления.                                      |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\ModuleManager`           | Компонент, ответственный за загрузку и инициализацию модулей веб-приложения. |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\EventManager`            | Компонент, обеспечивающий функциональность для инициации событий и обработки событий. |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\ServiceManager`          | Реестр всех сервисов, доступных в веб-приложении.             |
|--------------------------------|---------------------------------------------------------------|
...
I> Laminas состоит из сотен классов. Вы также будете писать свои собственные классы в ваших веб приложениях.
...
Если вы когда-нибудь разрабатывали сайты с Laminas Framework 1, вы, возможно, помните *супердлинные*
имена классов вроде `Laminas_Controller_Abstract`. Длинные имена использовались для того, чтобы
избежать конфликтов имен между различными компонентами. У каждого компонента был собственный префикс,
например, `Laminas_` или `My_`.

Laminas Framework для этих целей используют такую особенность языка PHP, как *пространства имен* (namespaces).
...
Ниже представлен пример определения пространства имен, взятый из компонента @`Laminas\Mvc`:
...
В Laminas Framework, все классы принадлежат пространству имен верхнего уровня *Laminas*.
Строка 2 определяет пространство имен *Mvc*, которое вложено в *Laminas*,
...
I> Обратите внимание на первый обратный слеш в имени @`\Laminas\Mvc\Application`
...
Интерфейсы широко распространены в Laminas Framework. Например, класс @`Application` реализует интерфейс
...
I> В Laminas Framework к именам интерфейсов принято добавлять суффикс `Interface`, например,
...
содержит определение класса @`\Laminas\Mvc\Application` из предыдущего раздела.
...
может вызывать трудности. Сам Laminas Framework состоит из сотен файлов, и
...
Например, для класса @`Laminas\Mvc\Application`, у вас будет следующая структура каталогов:
...
Например, если у вас есть полностью определенное имя класса @`\Laminas\Mvc\Application` и вы определяете соответствие
между @`\Laminas\Mvc` и каталогом "/path/to/laminas/laminas-mvc/src/", вы можете организовать файлы следующим образом:
...
I> Laminas Framework работает в соответствии со стандартом PSR-4, что делает возможным
...
Laminas Framework использует реализацию механизмов автозагрузки, предоставляемую Composer'ом. При установке пакетов
...
В Laminas Skeleton Application вы можете наблюдать стандарт PSR-4 на практике.
...
находящийся либо в библиотеке Laminas Framework, либо в вашем приложении, без необходимости использования
оператора `require_once`.

И в-третьих, в строке 40 создается экземпляр класса @`Laminas\Mvc\Application`.
...
Как вы поняли из предыдущего раздела, с каждым HTTP-запросом создается объект @`Laminas\Mvc\Application`.
...
I> Laminas Framework использует понятие *событие*. Класс может *вызвать* событие,
...
I> внутри компонента @`Laminas\EventManager`.

Каждый этап жизненного цикла приложения инициируется приложением путем вызова события (это событие
представлено классом @`MvcEvent`, который "живет" в пространстве имен @`Laminas\Mvc`). Другие классы
(принадлежащие либо Laminas Framework, либо вашему приложению) могут обработать события и отреагировать
...
класса @`Laminas\Router\Http\TreeRouteStack`). Если удается найти полное совпадение между URL и маршрутом,
...
@`Laminas\View\Renderer\PhpRenderer`. Визуализатор использует файл
...
Большинство компонентов Laminas Framework, используемых на вашем сайте, требуют
...
I> В Laminas Framework используется принцип *конфигурация превыше соглашений*,
...
return [
   ...
   // Должна быть совместима с Laminas\ServiceManager\Config.
   // 'service_manager' => [],
];
~~~
...
В строке 11 находится ключ `module_paths`, который сообщает Laminas о
...
Наконец, строка 19 содержит ключ `config_glob_paths`, который сообщает Laminas, где искать
...
компоненты Laminas Framework.

I> В Laminas был представлен специальный плагин Composer'a, называемый *установщиком компонентов*. Если помните, в
главе [Laminas Skeleton Application](#skeleton), мы отвечали на несколько вопросов об установщике, определяя,
...
*режима разработки*. Если помните, мы включали этот режим ранее в главе [Laminas Skeleton Application](#skeleton).
...
переопределить некоторые параметры. Например, вы можете:
  ...
  * загрузить дополнительные модули. Например, вы можете загрузить модуль [LaminasDeveloperTools](https://github.com/laminas/LaminasDeveloperTools) только в режиме разработки.
...
В Laminas ваше приложение состоит из модулей. По умолчанию у вас есть один единственный модуль `Application`, но вы можете создать
...
При старте создается объект @`Laminas\Mvc\Application`, и он использует компонент @`Laminas\ModuleManager`, чтобы найти и загрузить
...
Метод `getConfig()` обычно используется, чтобы предоставить Laminas Framework'у конфигурацию модуля (файл *module.config.php*).
...
В Laminas Framework класс @`ServiceManager` - это централизованный *контейнер* для всех
сервисов приложения. Менеджер сервисов  реализован в компоненте
@`Laminas\ServiceManager` в качестве класса @`ServiceManager`. Диаграмма наследования классов приведена
на рис. 3.5 ниже:
...
класса @`Laminas\Mvc\Application`).
...
|----------------------|-----------------------------------------------------------------------|
| `Application`          | Позволяет извлечь единственный экземпляр класса `Laminas\Mvc\Application`.|
|----------------------|-----------------------------------------------------------------------|
...
|----------------------|-----------------------------------------------------------------------|
| `EventManager`       | Позволяет извлечь *новый* экземпляр класса @`Laminas\EventManager\EventManager`. |
|                      | Менеджер событий позволяет вызывать события и прикреплять обработчики. |
|----------------------|-----------------------------------------------------------------------|
| `SharedEventManager` | Позволяет извлечь единственный экземпляр класса @`Laminas\EventManager\SharedEventManager`. Общий (shared)  |
|                      | менеджер событий позволяет слушать события, определенные в других классах и компонентах. |
|----------------------|-----------------------------------------------------------------------|
| `ModuleManager`        | Позволяет извлечь единственный экземпляр класса @`Laminas\ModuleManager\ModuleManager`. |
|                      | Менеджер модулей отвечает за загрузку модулей приложения.             |
|----------------------|-----------------------------------------------------------------------|
| `Request`              | Единственный экземпляр класса @`Laminas\Http\Request`. Представляет собой HTTP-запрос, |
|                      | полученный от клиента.                                                |
|----------------------|-----------------------------------------------------------------------|
| `Response`             | Единственный экземпляр класса @`Laminas\Http\Response`. Представляет собой HTTP-ответ,  |
|                      | который будет отправлен клиенту.                                      |
|----------------------|-----------------------------------------------------------------------|
| `Router`               | Единственный экземпляр класса @`Laminas\Router\Http\TreeRouteStack`. Осуществляет маршрутизацию URL. |
|----------------------|-----------------------------------------------------------------------|
| `ServiceManager`       | Собственно, сам менеджер сервисов.                                    |
|----------------------|-----------------------------------------------------------------------|
| `ViewManager`          | Единственный экземпляр класса @`Laminas\Mvc\View\Http\ViewManager`. Отвечает за |
|                      | подготовку слоя представления к визуализации страницы.                |
|----------------------|-----------------------------------------------------------------------|

Сервисом обычно является произвольный PHP-класс, но это не всегда так. Например, когда Laminas
...
Из таблицы 3.1 видно, что в Laminas все может считаться сервисом - даже сам менеджер сервисов.
...
I> Если у вас есть опыт с Laminas Framework 2, вы можете заметить, что все несколько изменилось.
...
I> (в контроллерах, других сервисах, и т.д.) В Laminas же вам придется передавать зависимости принудительно (explicitly). Это немного более утомительно,
...
типа класса @`Laminas\ServiceManager\ServiceManager` и была объявлена где-то еще).
...
в вашу собственную фабрику. Как правило, класс фабрики реализует @`FactoryInterface`[Laminas\ServiceManager\Factory\FactoryInterface]:
...
Как видно из определения @`FactoryInterface`[Laminas\ServiceManager\Factory\FactoryInterface], фабричный класс должен предоставлять магический
...
I> Технически, в Laminas вы *можете* использовать один и тот же класс фабрики для инстанциации нескольких сервисов, которые
...
фабрики должен определять интерфейс @`AbstractFactoryInterface`[Laminas\ServiceManager\Factory\AbstractFactoryInterface]:
...
Зачем это может понадобиться? На самом деле, менеджеры плагинов широко используются в Laminas, так как они
...
время и память). Существуют отдельные менеджеры плагинов для:
  ...
  * контроллеров (класс @`Laminas\Mvc\Controller\ControllerManager`)
  * плагинов контроллеров (класс @`Laminas\Mvc\Controller\PluginManager`)
  * помощников представлений (класс @`Laminas\View\HelperPluginManager`)
  * элементов форм (класс @`Laminas\Form\FormElementManager\FormElementManagerV3Polyfill`)
  * фильтров (класс @`Laminas\Filter\FilterPluginManager`)
  * валидаторов (класс @`Laminas\Validator\ValidatorPluginManager`)
  * и, возможно, чего-то другого

Так как любой менеджер плагинов наследуется от базового класса @`Laminas\ServiceManager\ServiceManager`,
...
вызов "callback" метода другого класса. Менеджер событий реализован внутри компонента @`Laminas\EventManager`.

T> Важно знать меру в использовании событий. Например, Laminas (и в частности его компонент @`Laminas\Mvc`) сильно зависит от событий в своей работе,
T> и по этой причине его исходный код - это "довольно сложная для понимания" комбинация обработчиков событий.
T> К счастью, в большинстве случаев вам не нужно понимать как Laminas вызывает события и реагирует на них внутри своего кода, вам только нужно знать
...
Событие (*event*) - это технически экземпляр класса @`Laminas\EventManager\Event`.
...
Также возможно создавать производные типы событий, расширяя класс @`Event`[Laminas\EventManager\Event].
Например, компонент @`Laminas\Mvc` определяет производный класс события @`Laminas\Mvc\MvcEvent`,
который наследуется от класса @`Event`[Laminas\EventManager\Event] и добавляет некоторые свойства и методы, которые нужны
для работы компонента @`Laminas\Mvc`.
...
В этой главе мы ознакомились с теорией основ работы сайта на базе Laminas.

Laminas использует пространства имен PHP и автозагрузку классов, упрощая тем самым
...
Большинство компонентов Laminas Framework требуют конфигурации. Вы можете определить параметры конфигурации либо
...
Для каждого HTTP-запроса создается объект @`Laminas\Mvc\Application`, чей "жизненный цикл" состоит из нескольких
этапов (или событий).

Бизнес-логику веб-приложения можно рассматривать как набор сервисов. В Laminas Framework менеджер сервисов -
