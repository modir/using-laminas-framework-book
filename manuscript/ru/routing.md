...
контроллера. В этой главе мы рассмотрим, как приложение на базе Laminas устанавливает соответствие
между URL и контроллерами и их действиями. Установление соответствия выполняется с помощью маршрутизации (routing).
Маршрутизация реализована как часть компонента @`Laminas\Router`.
...
| *Компонент*                    | *Описание*                                                    |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\Router`                  | Реализует поддержку маршрутизации.                            |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\Barcode`                 | Вспомогательный компонент, реализующий штрих-коды.            |
|--------------------------------|---------------------------------------------------------------|
...
"path", "to", и "page" будут его сегментами. Далее, после
...
разделенных друг от друга амперсандом ('&'). Наконец, после решетки ('#'), идет имя фрагмента.
...
Благодаря маршрутизации, Laminas знает, какой метод действия контроллера следует выполнить в результате
...
Laminas Framework предоставляет несколько стандартных типов маршрутов (см. таблицу 5.1).
Эти типы реализованы в качестве классов и находятся в пространстве имен @`Laminas\Router\Http`[Laminas\Router].
...
@`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack] и @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack] используются как типы маршрутов "верхнего уровня".
...
Как видите, все классы маршрутов наследуются от интерфейса @`RouteInterface`[Laminas\Router\RouteInterface] (мы
детально изучим этот интерфейс в разделе *Написание своего типа маршрута* позже
в этой главе). Класс @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack] - родительский для класса @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack], который
наследует поведение *SimpleRouteStack* (объединение маршрутов в список с приоритетами) и
расширяет его (объединение маршрутов в поддеревья). Классы @`Part`[Laminas\Router\Http\Part] и @`Chain`[Laminas\Router\Http\Chain]
наследуются от класса @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack] и используются @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack] внутренне для построения
поддеревьев и цепей дочерних маршрутов.

### SimpleRouteStack

Тип @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack] позволяет объединить различные маршруты в список по степени важности.
...
При сопоставлении с  HTTP-запросом, @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack] проходит через список
...
Класс @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack] расширяет @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack]. Это означает, что помимо возможности
...
Список состоит из одного маршрута @`Literal`[Laminas\Router\Http\Literal], цепи маршрутов @`Literal`[Laminas\Router\Http\Literal] и @`Segment` и
поддерева, состоящего из двух ветвей: ветви, содержащий один маршрут `Segment` и
ветви, состоящей из маршрутов @`Scheme`, @`Hostname`[Laminas\Router\Http\Hostname] и @`Segment`.
...
Если какой-либо из пунктов - это маршрут @`Chain`[Laminas\Router\Http\Chain] или @`Part`[Laminas\Router\Http\Part], @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack] обрабатывает этот вложенный маршрут от родительского
...
Как правило, вы не создаете описанные выше классы сами. Вместо этого вы сообщаете Laminas,
как это сделать.
...
Вы можете указать, какой класс маршрутов верхнего уровня использовать (@`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack] или @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack])
через параметр `router_class` (строка 7). Если такого параметра нет,
используется @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack].
...
T> Посмотрев на оба примера выше, вы не увидите явного использования типов @`Part`[Laminas\Router\Http\Part] и
T> @`Chain`[Laminas\Router\Http\Chain], потому что они (для вашего удобства) используются Laminas автоматически при
T> встрече с ключами `child_routes` и `chain_routes` в конфигурации маршрутизации.

### Конфигурация маршрутизации по умолчанию в Laminas Skeleton Application

Теперь, когда вы знаете, как настраивать маршруты и организовывать их в составные структуры, давайте
рассмотрим реальный пример. В чистом Laminas Skeleton Application, конфигурация
...
сопоставления маршрута реализован в классе @`Laminas\Router\Http\Literal`.
...
I> Чтобы иметь возможность использовать штрих-коды, вам нужно установить компонент @`Laminas\Barcode` через Composer,
...
Для генерации изображения штрих-кода мы используем компонент @`Laminas\Barcode`.
В строке 14 мы определяем текст маркировки штрих-кода. В строках 18-19 мы
создаем объект @`Barcode`[Laminas\Barcode\Barcode] с фабричным методом. Наконец, в строке 23 мы
выводим изображение, помещая его в поток вывода.

I> @`Laminas\Barcode` - это вспомогательный компонент, используемый для генерации
I> различных изображений штрих-кодов. Для дополнительной информации об этом компоненте,
I> смотрите соответствующий раздел руководства Laminas Framework.

В строке 26 мы возвращаем объект @`Response`[Laminas\Http\PhpEnvironment\Response], чтобы пресечь визуализацию стандартного представления.
...
если его нет, возвращаем код состояния 404 "Not Found", который принуждает Laminas отобразить
страницу ошибки. В строке 12 мы создаем контейнер для переменных @`ViewModel`[Laminas\View\Model\ViewModel], а в строке 15
...
URL генерируется с помощью помощника видов @`Url`[Laminas\View\Helper\Url] (подробнее о помощнике @`Url`[Laminas\View\Helper\Url] смотрите в следующих
разделах этой главы).
...
При удачном сопоставлении класс маршрутизатора внутренне создает экземпляр класса @`Laminas\Router\RouteMatch`,
предоставляя методы для извлечения имени соответствующего маршрута и полученных из него параметров.
Полезные методы класса @`RouteMatch`[Laminas\Router\RouteMatch] перечислены в таблице 5.3:
...
I> вы также можете использовать для этой цели объект @`RouteMatch`[Laminas\Router\RouteMatch].

Для получения объекта @`RouteMatch`[Laminas\Router\RouteMatch] из метода действия контроллера, используйте следующий
код:
...
извлечения объекта @`MvcEvent`, который представляет собой событие (В Laminas жизненный цикл приложения состоит
из событий). Затем мы используем метод `getRouteMatch()` класса @`MvcEvent` для извлечения объекта @`RouteMatch`[Laminas\Router\RouteMatch].
...
Этот интерфейс является базовым для @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack] и @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack] и предоставляет
...
Чтобы сгенерировать URL в шаблонах представления (файлы *.phtml*), можно использовать класс помощника вида @`Url`[Laminas\View\Helper\Url],
...
Здесь мы генерируем два относительных URL. В строке 2 мы вызываем помощник вида @`Url`[Laminas\View\Helper\Url] и передаем в
качестве параметра имя маршрута "home". В строке 5 мы передаем имя маршрута "about" в качестве аргумента
для помощника вида @`Url`[Laminas\View\Helper\Url].

I> В примере выше помощник вида @`Url`[Laminas\View\Helper\Url] внутренне использует объект @`RouteMatch`[Laminas\Router\RouteMatch] и вызывает
I> маршрут типа @`Literal`[Laminas\Router\Http\Literal], чтобы собрать строку URL с помощью имени маршрута.
...
Если среди параметров, используемых маршрутом, есть переменные, их следует передать помощнику вида @`Url`[Laminas\View\Helper\Url]
...
В этом примере мы используем помощник вида @`Url`[Laminas\View\Helper\Url] для генерации двух
...
вы можете указать третий параметр для помощника вида @`Url`[Laminas\View\Helper\Url]. Третий параметр
...
качестве третьего аргумента для помощника вида @`Url`[Laminas\View\Helper\Url]. Например, предположим, что у вас
...
@`Url`[Laminas\Mvc\Controller\Plugin\Url]. Чтобы сгенерировать URL, нужно вызвать метод `fromRoute()` данного плагина,
...
T> Аргументы, принимаемые плагином @`Url`[Laminas\Mvc\Controller\Plugin\Url], а также их значение, идентичны аргументам помощника вида `Url`[Laminas\View\Helper\Url].
T> Таким образом, вы можете сгенерировать абсолютные или относительные URL так же, как делали это в шаблонах представлений.

### Кодирование URL

При генерации URL как через помощник вида @`Url`[Laminas\View\Helper\Url], так и через плагин контроллера `Url`[Laminas\Mvc\Controller\Plugin\Url], следует
...
Хотя Laminas предоставляет вам множество типов маршрута, в некоторых ситуациях
...
Как мы знаем, каждый класс маршрута должен реализовывать интерфейс @`Laminas\Router\Http\RouteInterface`.
...
Статический метод `factory()` используется маршрутизатором Laminas (@`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack] или @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack])
...
либо экземпляр класса @`RouteMatch`[Laminas\Router\RouteMatch] при успешном сопоставлении, либо `null` при неудаче.
...
* Класс `StaticRoute` должно быть возможно вставить в стек маршрутов (либо в @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack],
  либо в @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack]) и использовать вместе с другими типами маршрутов.
...
реализует интерфейс @`RouteInterface`[Laminas\Router\RouteInterface] и определяет все методы, указанные
...
HTTP-запроса (экземпляр класса @`Laminas\Stdlib\Request`) и смещение
...
В строках 44-48 мы подготавливаем и возвращаем объект @`RouteMatch`[Laminas\Router\RouteMatch] с параметрами по умолчанию,
...
`$pageTemplate`. В строке 11 мы создаем контейнер для переменных @`ViewModel`[Laminas\View\Model\ViewModel], а в
...
Типы маршрутов могут быть объединены либо во вложенное дерево с помощью маршрутизатора @`TreeRouteStack`[Laminas\Router\Http\TreeRouteStack],
либо в цепь с помощью @`SimpleRouteStack`[Laminas\Router\SimpleRouteStack]. Эти два маршрутизатора позволяют определить
