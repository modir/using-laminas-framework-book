...
| *Компонент*                    | *Описание*                                                    |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\Mvc`                     | Поддержка шаблона MVC. Реализует базовые классы контроллера, плагины контроллера и т.д. |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\View`                    | Реализует функциональность контейнеров для переменных, рендеринга веб-страниц и помощников видов. |
|--------------------------------|---------------------------------------------------------------|
| @`Laminas\Http`                    | Реализует обертку HTTP-запроса и ответа.                      |
|--------------------------------|---------------------------------------------------------------|
...
Чтобы скачать приложение *Hello World* посетите [эту страницу](https://github.com/olegkrivtsov/using-laminas-book-samples)
...
* PHP-движок веб-сервера запускает входной скрипт *index.php*. Все, что делает этот скрипт, -
  это создает экземпляр класса @`Laminas\Mvc\Application`.
...
T> Laminas автоматически распознает методы действия по суффиксу 'Action'.
...
| *Имя метода*                     | *Описание*                                             |
|----------------------------------|--------------------------------------------------------|
| `getRequest()`                   | Возвращает объект @`Laminas\Http\Request`, который   |
|                                  | является представлением данных HTTP-запроса.           |
|----------------------------------|--------------------------------------------------------|
| `getResponse()`                  | Возвращает объект @`Laminas\Http\PhpEnvironment\Response`,  |
|                                  | что позволяет установить данные HTTP-ответа.          |
|----------------------------------|--------------------------------------------------------|
| `getEventManager()`              | Возвращает объект @`Laminas\EventManager\EventManager`,    |
|                                  | что позволяет вызвать и обработать событие.           |
|----------------------------------|--------------------------------------------------------|
| `getEvent()`                     | Возвращает объект @`Laminas\Mvc\MvcEvent` object, который  |
|                                  | представляет событие, на которое отвечает контроллер.  |
|----------------------------------|--------------------------------------------------------|
| `getPluginManager()`             | Возвращает объект @`Laminas\Mvc\Controller\PluginManager`, |
...
(такие данные как переменные GET и POST, куки (cookies), HTTP-заголовки и так далее). Для этих целей, Laminas Framework
предоставляет вам класс @`Laminas\Http\Request`, являющийся частью компонента @`Laminas\Http`.
...
Этот код возвращает экземпляр класса @`Laminas\Http\Request`, содержащий все данные HTTP-запроса.
В таблице 4.3 вы найдете самые распространенные методы класса 'Request`, а также их
краткое описание.

{title="Таблица 4.3. Методы класса @`Laminas\Http\Request`."}
|----------------------------------------|------------------------------------------------------|
| Имя метода                             | Описание                                             |
|----------------------------------------|------------------------------------------------------|
...
|----------------------------------------|------------------------------------------------------|
| `fromString($string)`                  | Статический метод, который создает объект @`Request`[Laminas\Http\Request] из |
...
T> В Laminas вы не должны обращаться к параметрам через стандартные суперглобальные массивы `$_GET` и `$_POST`
T> Вместо этого вы используете предоставляемое Laminas API для извлечения данных запроса.
...
Метод `getResponse()` возвращает экземпляр класса @`Laminas\Http\PhpEnvironment\Response`.
Таблица 4.4 содержит самые важные методы этого класса:

{title="Таблица 4.4. Методы класса Laminas\Http\PhpEnvironment\Response."}
|----------------------------------------|--------------------------------------------------------|
| Имя метода                             | Описание                                               |
|----------------------------------------|--------------------------------------------------------|
...
Как вы можете видеть, метод `indexAction()` Index-контроллера возвращает экземпляр класса @`ViewModel`[Laminas\View\Model\ViewModel].
Класс @`ViewModel`[Laminas\View\Model\ViewModel] - это своего рода *переменная-контейнер*. Все переменные, переданные в ее конструктор,
...
переменные для передачи в представление с помощью объекта @`ViewModel`[Laminas\View\Model\ViewModel]:
...
В строках 9-12 мы передаем переменные, которые мы создали, в конструктор объекта @`ViewModel`[Laminas\View\Model\ViewModel]
...
Класс @`ViewModel`[Laminas\View\Model\ViewModel] предоставляет несколько методов, которые вы можете дополнительно использовать,
чтобы установить переменные в @`ViewModel`[Laminas\View\Model\ViewModel] и извлечь переменные из него. Таблица 4.5
...
Когда Laminas обнаруживает код ошибки *4xx* в ответе, он перенаправляет пользователя на специальную *страницу ошибки*. Мы поговорим
...
Когда Laminas обнаруживает необработанное исключение, он показывает другую страницу ошибки с информацией об исключении.
...
I> Используя @`InvokableFactory`, вы говорите Laminas Framework'у, что
...
I> Если у вас есть опыт работы с Laminas Framework 2, вы можете заметить, что в Laminas все стало немного по-другому.
...
I> В Laminas вы должны передавать зависимости принудительно. Это немного более утомительно, но зато удаляет
...
I> наследовать другие контроллеры от этого базового класса. Так тоже можно делать, но с точки зрения создателей Laminas плагины -
...
I> контроллеров вашего приложения (базовый класс @`AbstractActionController` использует магический PHP-метод `__call()`
...
|------------------------------------------|------------------------------------------------------|
| @`Url`[Laminas\View\Helper\Url]                                    | Позволяет генерировать абсолютные или относительные  |
|                                          | URL изнутри контроллеров.                            |
|------------------------------------------|------------------------------------------------------|
| @`Layout`[Laminas\View\Helper\Layout]                                 | Дает модели представления лэйаута доступ для         |
|                                          | передачи данных шаблону лэйаута.                     |
|------------------------------------------|------------------------------------------------------|
| @`Identity`[Laminas\View\Helper\Identity]                               | Возвращает личность (identity) пользователя, который зашел на   |
|                                          | вебсайт.                                             |
|------------------------------------------|------------------------------------------------------|
| @`FlashMessenger`[Laminas\View\Helper\FlashMessenger]                         | Позволяет определить "flash"-сообщения, который хранятся|
...
Плагин контроллера должен наследоваться от класса @`AbstractPlugin`[Laminas\Mvc\Controller\Plugin\AbstractPlugin].
...
Чтобы дать Laminas Framework знать о вашем плагине,вам нужно зарегистрировать его в файле
...
В Laminas Framework, представление является *файлом шаблона*, то есть, файлом с разрешением
...
Q> классов PHP. Шаблоны представлений загружаются специальным классом Laminas, называемым
...
действия контроллера. Данные передаются шаблонам с помощью контейнера переменных @`ViewModel`[Laminas\View\Model\ViewModel].
...
Скрипт представления просто обрабатывает данные, которые вы ему передаете с помощью контейнера @`ViewModel`[Laminas\View\Model\ViewModel].
...
Laminas содержит множество стандартных помощников представления. В таблице 4.7 представлены некоторые
...
|------------------------------------------|------------------------------------------------------|
| @`Url`[Laminas\Mvc\Controller\Plugin\Url]                                    | Позволяет сгенерировать абсолютный или относительный |
...
|------------------------------------------|------------------------------------------------------|
| @`ViewModel`[Laminas\View\Model\ViewModel]                              | Помощник для хранения и извлечения модели представления.|
|------------------------------------------|------------------------------------------------------|
| @`Layout`[Laminas\Mvc\Controller\Plugin\Layout]                                 | Извлекает шаблон представления лэйаута.              |
|------------------------------------------|------------------------------------------------------|
...
|------------------------------------------|------------------------------------------------------|
| @`Identity`[Laminas\View\Helper\Identity]                               | Помощник для извлечения личности авторизированного пользователя.|
|------------------------------------------|------------------------------------------------------|
| @`FlashMessenger`[Laminas\Mvc\Plugin\FlashMessenger]                         | Позволяет извлечь "flash"-сообщения, хранящиеся в сессии.|
|------------------------------------------|------------------------------------------------------|
...
Когда вы возвращаете данные с контейнером переменных @`ViewModel`[Laminas\View\Model\ViewModel] из метода действия
вашего контроллера, Laminas Framework знает имя соответствующего файла шаблона представления.
Например, для метода `aboutAction()` вашего `IndexController`'а, Laminas автоматически использует
шаблон представления *about.phtml*.

I> Laminas определяет корректное имя шаблона представления по имени модуля, имени контроллера и имени действия.
...
@`ViewModel`[Laminas\View\Model\ViewModel] также может использоваться, чтобы переопределить стандартные имена шаблонов представления.
Вообще, класс @`ViewModel`[Laminas\View\Model\ViewModel] - это больше, чем просто контейнер для переменных. Он также позволяет переопределить,
...
Laminas использовать файл шаблона представления *about.phtml* для визуализации главной страницы
...
В приведенном отрезке кода мы как обычно создали новый экземпляр класса @`ViewModel`[Laminas\View\Model\ViewModel] (строка 5).
...
Если вы не сделаете этого, Laminas определит имя шаблона представления автоматически,
по текущему имени модуля, имени контроллера и имени метода действия.

## View Resolver

Когда у Laminas Framework есть имя шаблона, ему остается только определить абсолютный путь
к соответствующему файлу *.phtml*. Это также называется *разрешением представлений (view resolving)*.
Шаблоны разрешаются специальным классом Laminas Framework, который называется *view resolver* (разрешатель представлений)

В Laminas существуют два разрешателя представлений: @`TemplatePathStack` и @`TemplateMapResolver`.
...
Поэтому мы можем просто положить файл *about.phtml* в эту директорию, и Laminas автоматически
найдет шаблон.
...
Чтобы это сделать, нужно просто вернуть объект @`Response`[Laminas\Http\PhpEnvironment\Response] из действия контроллера.
...
объекту @`Response`[Laminas\Http\PhpEnvironment\Response] (строки 39-45) и содержимое файла (строки 48-55). Наконец,
он возвращает объект @`Response`[Laminas\Http\PhpEnvironment\Response], чтобы отключить рендеринг представления.
...
живущего в простанстве имен @`Laminas\View\Renderer`[Laminas\View]. Эта стратегия работает хорошо в 99% случаев. Но иногда вам может
...
Итак, Laminas из коробки предлагает три стратегии рендеринга представления:

  * стратегия, используемая по умолчанию (также известная как @`PhpRendererStrategy`).
...
Затем верните объект @`JsonModel` (вместо обычного @`ViewModel`[Laminas\View\Model\ViewModel]) из метода действия контроллера:
...
I> С Laminas вы представляете модели как обычные PHP-классы.
Свойства представлены как поля класса, а поведения - как методы.

## Типы моделей

В Laminas Framework, нет одной единственной директории 'Model' для хранения классов моделей, как вы
...
T> В Laminas Framework вы, как правило, регистрируете ваши сервисы в менеджере сервисов (Service Manager).
...
@`Laminas\ServiceManager\Factory\FactoryInterface`. У класса фабрики есть метод `__invoke()`,
...
* и наконец, возвращает выходные данные как часть контейнера @`ViewModel`[Laminas\View\Model\ViewModel].
...
* Принимает данные, переданные посетителем сайта (строка 16). Данные обычно являются частью
  объекта @`Request`[Laminas\Http\Request] и могут быть извлечены с использованием метода контроллера `getRequest()`
...
* Создает переменную-контейнер @`ViewModel`[Laminas\View\Model\ViewModel] и передает ей полученные данные (строка 28).
...
Вебсайт на Laminas Framework - это всего лишь PHP-программа, получающая HTTP-запрос
...
контейнер переменных @`ViewModel`[Laminas\View\Model\ViewModel].
